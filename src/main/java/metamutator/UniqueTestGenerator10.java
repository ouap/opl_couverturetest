...
public class UniqueTestGenerator10 extends AbstractProcessor<CtClass> {
	...
	public boolean isToBeProcessed(CtClass element) {
		System.out.println(element.getElements(new AnnotationFilter<>(Test.class)).size()>0
	...
	public void process(CtClass element) {
		...
		try {
			...
			if ((val > seuil) && (cptMethods + element.getMethodsAnnotatedWith(getFactory().Annotation().createReference(Test.class)).size() < 10)) {
